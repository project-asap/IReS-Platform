...
mem_kb = popen("vmstat -s | grep 'total mem' | awk '{print $1}'").read()
my_cores = int(popen("grep -c ^processor /proc/cpuinfo   ").read())
my_mem_gb=int(ceil(float(mem_kb)/1024/1024))
my_disc_count=1

print "Assumming single disk"
...
# print
# print "%d containers"%container_count
...
def main():
  ...
  disks = 0
  hbaseEnabled = True
  parser.add_option('-c', '--cores', default = my_cores,
                     help = 'Number of cores on each host')
  parser.add_option('-m', '--memory', default = my_mem_gb,
                    help = 'Amount of Memory on each host in GB')
  parser.add_option('-d', '--disks', default = my_disc_count,
