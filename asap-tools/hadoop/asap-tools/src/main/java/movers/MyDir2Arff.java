 ...
 * @see Loader
 */
public class MyDir2Arff extends AbstractLoader implements
  ...
  /**
   * The maximum number of instances to handle
   */
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    //cmantas: add count of documents to parse
    String maxInstancesIn = Utils.getOption("count", options);
    if (maxInstancesIn.length()>0)
        maxInstances = Long.parseLong(maxInstancesIn);
  ...
  public Instances getStructure() throws IOException {
    ...
    if (m_structure == null) {
      ...
      //cmantas: circumvent classes and just add "text"
      classes.add("text");
      ...
      // clash with any attribute created via the StringToWordArrayList filter
  ...
  public Instance getNextInstance(Instances structure) throws IOException {
    ...
    if (m_filesByClass == null) {
      ...
      for (int i = 0; i < classAtt.numValues(); i++) {
        /*cmantas: replace 
            directoryPath + File.separator + classAtt.value(i) 
          with just directoryPath
          */
        File classDir = new File(directoryPath);
        String[] files = classDir.list();
        
        LinkedList<String> classDocs = new LinkedList<String>();
        for (String cd : files) {
          File txt =
            new File(directoryPath + File.separator + cd);
          if (txt.isFile()) {
            classDocs.add(cd);
          }
        }
    ...
    if (found) {
      ...
      File txt =
        new File(directoryPath + File.separator + nextDoc);
  ...
  public void run(Object toRun, String[] args) throws IllegalArgumentException {
    ...
    if (args.length > 0) {
      try {
        ...
        } while (temp != null && count<loader.maxInstances);
      } catch (Exception e) {
        e.printStackTrace();
      }
    } else {
      ...
      Enumeration<Option> enm =
        ((OptionHandler) new MyDir2Arff()).listOptions();
      while (enm.hasMoreElements()) {
