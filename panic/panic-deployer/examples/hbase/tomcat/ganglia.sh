...
TOMCAT=$(cat /etc/hosts | grep "tomcat1$" | awk '{print $1}')
sed -i -e "/^udp_send_channel/ a \  host=$TOMCAT" /etc/ganglia/gmond.conf
...
sed -i -e "/^cluster {/ a \  name=\"tomcat\"" /etc/ganglia/gmond.conf
