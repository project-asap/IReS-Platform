	...
	do
		...
		do
			...
			do
				...
				pdcp -R ssh -w slave[1-10] $HOME/optimizingJoins/k.txt /home/hadoop/optimizingJoins/
...
#				. resources/experiments/preprocessing.sh $jointype ${preprocess[$1]}
				. preprocessing.sh $jointype ${preprocess[$1]}
				...
				do
					...
					then
						...
						then 
							...
							while true
							do
								safe=`$HOME_YARN_FORMAT dfsadmin -safemode get | grep OFF | awk '{print $NF}'`
								if [ "$safe" == "OFF" ]
								then
									break
								fi
