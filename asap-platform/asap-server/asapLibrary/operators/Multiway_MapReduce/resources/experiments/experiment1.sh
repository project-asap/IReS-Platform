...
experiment1()
{
	...
	do
		...
		relations=`wc -l $HOME/$jointype/$jointype.txt | awk '{print $1}'`
		...
		do
			...
			do
				...
				input=$records
				args=( $jointype/$jointype.txt  $records $mb $input $relations)
			
				$YARN_JAR_EXECUTION jar ${jar[$1]} ${class[$1]} ${genericoptions[@]} ${args[@]}
			done
		done
	done
}
