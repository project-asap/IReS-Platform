...
preprocessing()
{
	...
	$HOME_HADOOP_EXECUTION fs -rmr /user/hadoop/optimizingJoins/$jointype
	echo
	...
	if [ ! -e "$join_home" ]
		then
			...
			for slave in ${SLAVES[@]}
			do
				scp resources/joins/$jointype.txt slave$slave:/home/hadoop/$jointype
			done
	fi
	
	#run the preprocessing step for this jointype
	$HOME_HADOOP_EXECUTION jar $HOME/PreProc.jar PreProcessing $jointype/$jointype.txt $multi_or_two_processing
	...
	pdcp -R ssh -w slave[${SLAVES[0]}-${SLAVES[${#SLAVES[@]} - 1]}] $jointar $join_home
	pdsh -R ssh -w slave[${SLAVES[0]}-${SLAVES[${#SLAVES[@]} - 1]}] "tar -xf $join_home/$jointype.tar -C $join_home"
