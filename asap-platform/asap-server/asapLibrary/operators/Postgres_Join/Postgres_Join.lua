SHELL_COMMAND = "./Postgres_Join.sh"
...
base_env = {
  CLASSPATH = table.concat({"${CLASSPATH}", CP, "./master.jar", "./Postgres_Join.sh"}, ":"),
}
...
operator = yarn {
  ...
  container = {
    ...
    resources = {
    ["Postgres_Join.sh"] = {
       file = "/opt/asap-server/asapLibrary/operators/Postgres_Join/Postgres_Join.sh",
