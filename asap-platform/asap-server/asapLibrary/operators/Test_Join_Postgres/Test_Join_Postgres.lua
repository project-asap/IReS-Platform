SHELL_COMMAND = "./Test_Join_Postgres.sh"
...
base_env = {
  CLASSPATH = table.concat({"${CLASSPATH}", CP, "./master.jar", "./Test_Join_Postgres.sh"}, ":"),
}
...
operator = yarn {
  ...
  container = {
    ...
    stageout = {"output"},
    resources = {
    ["Test_Join_Postgres.sh"] = {
       file = "/opt/asap-server/asapLibrary/operators/Test_Join_Postgres/Test_Join_Postgres.sh",
