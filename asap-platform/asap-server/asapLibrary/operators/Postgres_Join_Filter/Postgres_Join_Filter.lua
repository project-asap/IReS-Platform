SHELL_COMMAND = "./Postgres_Join_Filter.sh"
...
base_env = {
  CLASSPATH = table.concat({"${CLASSPATH}", CP, "./master.jar", "./Postgres_Join_Filter.sh"}, ":"),
}
...
operator = yarn {
  ...
  container = {
    ...
    ["Postgres_Join_Filter.sh"] = {
       file = "/opt/npapa/asap-server/asapLibrary/operators/Postgres_Join_Filter/Postgres_Join_Filter.sh",
