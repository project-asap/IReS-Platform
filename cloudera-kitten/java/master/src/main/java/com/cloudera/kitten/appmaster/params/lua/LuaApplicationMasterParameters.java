...
public class LuaApplicationMasterParameters implements ApplicationMasterParameters {
  ...
  public HashMap<String,ContainerLaunchParameters> getContainerLaunchParameters() {
    ...
    if (!env.isNil(LuaFields.CONTAINERS)) {
      ...
      while (iter.hasNext()) {
        clp.put("operator_"+i,new LuaContainerLaunchParameters(iter.next().value,"operator_"+i, conf, localToUris));
        i++;
      }
    } else if (!env.isNil(LuaFields.CONTAINER)) {
        clp.put("operator_0",new LuaContainerLaunchParameters(env.getTable(LuaFields.CONTAINER), "operator_0", conf, localToUris));
    }
    return clp;
  }
}
