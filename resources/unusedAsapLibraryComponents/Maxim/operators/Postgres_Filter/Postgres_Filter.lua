SHELL_COMMAND = "./Postgres_Filter.sh"
...
base_env = {
  CLASSPATH = table.concat({"${CLASSPATH}", CP, "./master.jar", "./Postgres_Filter.sh"}, ":"),
}
...
operator = yarn {
  ...
  container = {
    ...
    resources = {
    ["Postgres_Filter.sh"] = {
       file = "/opt/asap-server/asapLibrary/operators/Postgres_Filter/Postgres_Filter.sh",
