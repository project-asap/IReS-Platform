SHELL_COMMAND = "./Test_Filter_Postgres.sh"
...
base_env = {
  CLASSPATH = table.concat({"${CLASSPATH}", CP, "./master.jar", "./Test_Filter_Postgres.sh"}, ":"),
}
...
operator = yarn {
  ...
  container = {
    ...
    stageout = {"output"},
    resources = {
    ["Test_Filter_Postgres.sh"] = {
       file = "/opt/asap-server/asapLibrary/operators/Test_Filter_Postgres/Test_Filter_Postgres.sh",
